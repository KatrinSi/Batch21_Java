package day18_String;

public class String_Practice {
    public static void main(String[] args) {
        String s1 = "Cat";
        String s2 = "Cat";

        System.out.println(s1 == s2);//true

        String s3 = new String("Cat");
        System.out.println(s1 == s3);//false
        System.out.println(s2 == s3);//false

        String s4 = new String("Cat");
        System.out.println(s3 == s4);//false

        System.out.println(s1.equals(s3));//true (compares a text!!!)
        //.equals() !! to compare the text!!

        System.out.println(s2.equals(s3));//true
        System.out.println(s3.equals(s4));//true

        String s5 = new String("cat");
        System.out.println(s4.equals(s5));//false, equals method is case sensitive
        System.out.println(s4.equalsIgnoreCase(s5));//true

        System.out.println("===============================");

        String str = "cybertek";
        str = str.toUpperCase();//CYBERTEK

        System.out.println(str);


        System.out.println("==============================");

        String name = "Katrin Si";
        int n1 = name.length();
        System.out.println(n1);

        String name2 = "Muhtar";
        int n2 = name2.length();//6
        System.out.println(n2);

        int lastInd = name2.length()-1;//5
        System.out.println(lastInd);

        System.out.println("==========================");
        String a = "Java Programming Language";
        char firstChar = a.charAt(0);
        char lastChar = a.charAt(a.length()-1);//last index num of a
        System.out.println("First character: "+firstChar);
        System.out.println("Last chatacter: "+lastChar);

        char secondChar = a.charAt(1);
        char secondLastChar = a.charAt(a.length()-2);
        System.out.println("secondChar= " + secondChar);
        System.out.println("secondLastChar = " + secondLastChar);








    }
}
